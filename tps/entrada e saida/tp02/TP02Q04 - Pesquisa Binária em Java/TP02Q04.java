...
class TP02Q04 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String[] entradaPesq = new String[500];
        ...
        do {
            entradaPesq[numEntradaPesq] = MyIO.readLine();
        ...
        String[] gamesName = new String[games.length];

        for (int a = 0; a < games.length; a++) {
            gamesName[a] = (games[a].getName());

        }

        int size = gamesName.length;
        
        for (int a = 0; a < size - 1; a++) {
            for (int j = a + 1; j < gamesName.length; j++) {
                
                if (gamesName[i].compareTo(gamesName[j]) > 0) {
                    
                    String temp = gamesName[i];
                    gamesName[i] = gamesName[j];
                    gamesName[j] = temp;
                }
            }
        }
        ...
        for (int x = 0; x < numEntradaPesq; x++) {
            for (int y = 0; y < games.length; y++) {
                if ((games[y].getName().compareTo(entradaPesq[x])) == 0) {
                    resp = true;
                    y = numEntradaPesq;
